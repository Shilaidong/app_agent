...
assert(source.includes("select_option"), "CUA select_option action is missing")
assert(source.includes("resume_cua"), "CUA resume action is missing")
...
assert(source.includes("function normalizeApplicationUrl"), "Generated CUA tool must define normalizeApplicationUrl")
assert(source.includes("\"list_windows\""), "CUA list_windows action is missing")
for (const action of ["observe_page", "fill_field_verified", "select_option_verified", "save_page_verified", "handle_blocker"]) {
  assert(source.includes(`"${action}"`), `CUA high-level action is missing: ${action}`)
}
assert(source.includes("capture_mode: input.saveScreenshot ? \"som\" : \"ax\""), "CUA capture_state must default to AX-only to avoid screen-recording prompts")
assert(source.includes("isBeforeUnloadText"), "CUA beforeunload blocker detection is missing")
assert(source.includes("离开此网站"), "CUA beforeunload Chinese prompt handling is missing")
assert(source.includes("FIELD_FILLED_NEEDS_RECHECK"), "CUA verified field recheck response is missing")
assert(source.includes("SELECT_NEEDS_RECHECK"), "CUA verified select recheck response is missing")
assert(source.includes("SAVE_BLOCKED_REQUIRED_FIELDS"), "CUA save preflight required-field guard is missing")
assert(source.includes("UNVERIFIED_SAVE_RECORDED"), "record_saved must not mark a page as verified saved")
assert(source.includes("inspect_controls"), "CUA inspect_controls action is missing")
assert(source.includes("dom_set_field"), "CUA DOM field setter is missing")
assert(source.includes("enable_browser_dom"), "CUA browser DOM enable action is missing")
assert(source.includes("dismiss_dialog"), "CUA dismiss_dialog action is missing")
assert(source.includes("dismiss_native_menu"), "CUA native menu dismiss action is missing")
assert(source.includes("浏览器弹窗"), "CUA browser dialog SOP is missing")
assert(source.includes("keyboard_fill_sequence"), "CUA keyboard_fill_sequence fallback is missing")
assert(source.includes("nativeSelectTypeahead"), "CUA native select typeahead helper is missing")
assert(source.includes("原生菜单键盘兜底"), "CUA native select keyboard fallback is missing")
assert(source.includes("AX popup menu"), "CUA AX popup menu option fallback is missing")
assert(source.includes("replace"), "CUA replace-text option is missing")
assert(source.includes("domAutomationUnavailable"), "CUA DOM-unavailable state flag is missing")
assert(source.includes("nativeSelectHints"), "CUA inspect_controls must expose native select hints")
assert(source.includes("系统样式原生下拉框"), "Native select popup SOP is missing")
assert(source.includes("cua-driver"), "CUA driver integration is missing")
assert(source.includes("cua_control.json"), "CUA control state contract is missing")
assert(source.includes("execCua"), "CUA calls must go through the watchdog wrapper")
assert(source.includes("CUA_STOPPED"), "CUA manual stop circuit breaker is missing")
assert(source.includes("CUA_RATE_LIMITED"), "CUA rate-limit circuit breaker is missing")
assert(source.includes("CUA_CHROME_JS_DISABLED"), "CUA Chrome JavaScript Apple Events circuit breaker is missing")
assert(source.includes("允许 Apple 事件中的 JavaScript"), "CUA Chrome Apple Events guidance is missing")
assert(source.includes("isChromeJavaScriptAppleEventsDisabled"), "CUA Chrome Apple Events detector is missing")
assert(source.includes("consecutiveFailures >= 3"), "CUA consecutive-failure breaker is missing")
assert(source.includes("notifyCuaStopped"), "CUA automatic stop notification is missing")
...
if (workspace) {
  ...
  assert(tools.includes("select_option"), "Workspace CUA tool missing select_option")
  for (const action of ["observe_page", "fill_field_verified", "select_option_verified", "save_page_verified", "handle_blocker"]) {
    assert(tools.includes(action), `Workspace CUA tool missing high-level action: ${action}`)
  }
  assert(tools.includes("UNVERIFIED_SAVE_RECORDED"), "Workspace record_saved must require save_page_verified")
  assert(tools.includes("inspect_controls"), "Workspace CUA tool missing inspect_controls")
