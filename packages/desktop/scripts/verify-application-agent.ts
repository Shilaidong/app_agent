...
const opencodeMessageV2Source = readFileSync(join(root, "../opencode/src/session/message-v2.ts"), "utf8")
const opencodeRetrySource = readFileSync(join(root, "../opencode/src/session/retry.ts"), "utf8")
const opencodeOverflowSource = readFileSync(join(root, "../opencode/src/session/overflow.ts"), "utf8")
...
const viewModelSource = readFileSync(join(root, "src/renderer/application-agent-view-model.ts"), "utf8")
...
assert(modelSource.includes("applicationAgentThinkingConfig") && modelSource.includes("thinkingOptionsForModel") && modelSource.includes('thinking: { type: "enabled"') && modelSource.includes('variant: "high"') && modelSource.includes('reasoningEffort: "high"') && modelSource.includes("Only emits the active provider"), "Default sessions must enable thinking for the active provider only")
assert(modelSource.includes("思考模式开启"), "Catalog must advertise thinking for all models")
assert(source.includes("applicationAgentThinkingConfig") && source.includes("provider: thinking.provider") && source.includes("variant: thinking.variant"), "Workspace generator must apply active-provider thinking config and agent variant high")
assert(source.includes("kill_terra_managed_ego_residuals") && source.includes("has_external_ego_lite_process") && source.includes("has_ego_browser_launchd_service"), "Ego wrapper must clear Terra-managed leftovers and only EXTERNAL-gate true external Ego")
assert(guiDialogVerifySource.includes("purgeTerraManagedResiduals") && guiDialogVerifySource.includes("terraManagedResidualPids"), "GUI dialog smoke must auto-purge Terra-managed Ego leftovers without requiring PURGE_RESIDUALS")
assert(mainSource.includes("openApplicationAgentSession") && ipcSource.includes("application-agent:open-session"), "Reading existing tasks must support open-session with selected model")
...
assert(source.includes("本页落盘/前进控件") && (source.includes("该点击必须包在 observePageAction 里") || source.includes("必须通过 observePageAction 点击页面上真实可见的本页落盘") || source.includes("优先 clickPageCommitControl") || source.includes("必须用 clickPageCommitControl") || source.includes("必须用 TERRA_POLICY 的 clickPageCommitControl")), "ego-browser SOP must require a real page-commit click after verified page completion")
...
assert(source.includes("clickByCoordinates") && source.includes("clickPageCommitControl") && source.includes("scrollIntoView") && source.includes("inViewport") && source.includes("TERRA_EGO_SYNTHETIC_DOM_EVENT_DENIED") && source.includes("TERRA_EGO_UNAUTHORIZED_TAKEOVER"), "Coordinate CDP click fallback, page-commit scroll-into-viewport helper, synthetic DOM event ban, and unauthorized takeOver gate must ship")
assert(source.includes("selectOptionByKeyboard") && source.includes("click+keyboard-option+reobserve") && source.includes("TERRA_EGO_SELECT_FILLINPUT_DENIED") && source.includes("owner-select") && source.includes("frosted OS menu"), "Native select owner-select helper and fillInput-on-select ban must ship")
assert(source.includes("Stubborn page-commit ladder") && source.includes("SUSPECTED_ALERT_PATH_REQUIRED") && source.includes("SAVE_ATTEMPT_OPEN") && source.includes("NESTED_MODAL_BLOCKER") && source.includes("isNestedModalHandoffEvidence"), "Stubborn page-commit ladder, early-handoff gates, and NESTED_MODAL_BLOCKER exemption must ship")
assert(source.includes("网页二级模态框") && source.includes("Start New Application") && source.includes("waitForNestedModalForm") && source.includes("successMode: 'modal-closed'") && source.includes("nested_modal_closed") && source.includes("return to nested modal") && source.includes("坐标为 0 时禁止乱点"), "ego-browser SOP must require nested-modal-first ladder, modal-closed evidence, return-to-nested-modal after alert, and ban zero-rect shortcuts")
assert(source.includes("TERRA_EGO_MODAL_CANCEL_AFTER_FILL_DENIED") && source.includes("fillPortalLookupBySearch") && source.includes("nested_modal_open") && source.includes("successMode other than modal-closed"), "Modal Cancel-after-fill ban, lookup helper, and behavior-based nested_modal_open parent gate must ship")
assert(source.includes("本页可确认字段填完") && source.includes("禁止对小表 begin_save_attempt") && !/7\. 本页\/本模态框可确认字段填完[\s\S]{0,80}保存前先 begin_save_attempt/.test(source), "CUA rule 7 must not tell agents to begin_save_attempt on nested modals")
assert(source.includes("runtime rejects such laundering") || source.includes("Rejects evidence that still contains timeout wording"), "Nested handoff evidence must reject timed-out wording laundering")
...
assert(source.includes("Vue internals") && source.includes("fillInput+Tab+readback") && source.includes("click+snapshot+click-option+reobserve") && source.includes("selectOptionByKeyboard"), "Terra policy must require real generic interactions including native-select keyboard helper and ban framework-internal writes")
...
assert(rendererSource.includes("AgentDisplayRow") && rendererSource.includes("createMemo(() =>"), "Chat rows must use reactive memos (AgentDisplayRow) instead of snapshotting Show item()")
assert(rendererSource.includes('item().kind === "technical-group"') || rendererSource.includes("value.kind === \"technical-group\""), "Technical/message branch must read item kind reactively")
assert(rendererSource.includes("AgentTechnicalItem") && rendererSource.includes("messageId={id}"), "Tool rows must re-read message body/status by id across polls")
assert(source.includes("observePageAction(() => click(icon") && source.includes("observePageAction(() => click(selector"), "fillPortalLookupBySearch icon/field clicks must use observePageAction")
assert(source.includes("Reject any timeout-shaped wording"), "Nested handoff evidence must reject bare timed-out wording")
assert(opencodeOverflowSource.includes("Always subtract reserved"), "overflow.usable must apply compaction.reserved for context-only models")
assert(rendererSource.includes("maybeAutoRecoverModelStreamError") && rendererSource.includes("MODEL_STREAM_RECOVER_PROMPT"), "Renderer must auto-continue once after terminated model-stream errors")

assert(viewModelSource.includes("isRecoverableModelStreamError"), "View model must classify recoverable model-stream errors")
assert(opencodeMessageV2Source.includes("isTransientNetworkStreamError") && opencodeMessageV2Source.includes("Model stream connection terminated"), "OpenCode must map terminated streams to retryable APIError")
assert(opencodeRetrySource.includes("UnknownError") && opencodeRetrySource.includes("Model stream connection terminated"), "OpenCode retry policy must treat legacy UnknownError(terminated) as retryable")

assert(rendererSource.includes("scrollAgentToLatest"), "Renderer must keep the application chat pinned to its latest message")
assert(rendererSource.includes("distanceFromBottom"), "Renderer must use sticky chat scroll instead of forcing jump on every poll")
assert(rendererSource.includes("agentDisplayGroupIds"), "Renderer must key chat rows by stable display ids to avoid remount flash")
assert(/technical:\$\{technical\[0\]\.id\}/.test(viewModelSource), "Technical group ids must stay stable from the first tool id")
assert(mainSource.includes("formatOpenCodeSessionError") && mainSource.includes("error.data?.message"), "OpenCode errors must surface data.message instead of bare UnknownError")
...
assert(rendererSource.includes("openApplicationAgentSession(latestTask, selectedModelId())"), "Opening an existing or later school task must use openApplicationAgentSession with the selected model")
assert(mainSource.includes("if (!anyExisting) return startApplicationAgentSession"), "First entry into a school with no session must still send the startup prompt")
