...
assert(source.includes("selectOptionByKeyboard") && source.includes("click+keyboard-option+reobserve") && source.includes("TERRA_EGO_SELECT_FILLINPUT_DENIED"), "Native select keyboard helper and fillInput-on-select ban must ship")
assert(source.includes("Stubborn page-commit ladder") && source.includes("SUSPECTED_ALERT_PATH_REQUIRED") && source.includes("SAVE_ATTEMPT_OPEN"), "Stubborn page-commit ladder and early-handoff gates must ship")
assert(source.includes("网页二级模态框") && source.includes("Start New Application") && source.includes("click+snapshot+click-option+reobserve") && source.includes("坐标为 0 时禁止乱点"), "ego-browser SOP must keep in-page DOM modals on Ego @ref path and ban zero-rect coordinate shortcuts")
