...
assert(modelSource.includes("applicationAgentThinkingConfig") && modelSource.includes("thinkingOptionsForModel") && modelSource.includes('thinking: { type: "enabled"') && modelSource.includes('variant: "high"') && modelSource.includes('reasoningEffort: "high"'), "Default sessions must enable thinking for every curated model")
assert(modelSource.includes("思考模式开启") && modelSource.includes("Every curated model defaults to thinking on"), "Catalog must advertise thinking for all models")
assert(source.includes("applicationAgentThinkingConfig") && source.includes("provider: thinking.provider") && source.includes("variant: thinking.variant"), "Workspace generator must apply full-catalog thinking config and agent variant high")
