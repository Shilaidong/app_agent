...
function verifyWorkspace(workspace: string, expectPaused: boolean) {
  ...
  assert(wrapperSource.includes("has_ego_browser_launchd_service") && wrapperSource.includes("has_external_ego_lite_process") && wrapperSource.includes('/bin/launchctl print "gui/$USER_ID/com.citrolabs.ego.lite.ego-browser"'), "Workspace wrapper must classify external Ego via exact launchd print and process path, without grepping the whole gui domain.")
  assert(wrapperSource.includes("kill_terra_managed_ego_residuals") && wrapperSource.includes("has_terra_managed_ego_process"), "Workspace wrapper must clear Terra-managed Ego leftovers before EXTERNAL/SERVICE gates.")
