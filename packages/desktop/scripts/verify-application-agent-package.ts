...
assert(asarBytes.includes(Buffer.from("TERRA_EGO_SCRIPTED_SUBMIT_DENIED")) && asarBytes.includes(Buffer.from("TERRA_EGO_SAVE_MUST_USE_OBSERVE_PAGE_ACTION")) && asarBytes.includes(Buffer.from("TERRA_EGO_NATIVE_ALERT_CLICK_DENIED")) && asarBytes.includes(Buffer.from("TERRA_EGO_ALERT_MUST_END_ROUND")) && asarBytes.includes(Buffer.from("TERRA_EGO_SYNTHETIC_DOM_EVENT_DENIED")) && asarBytes.includes(Buffer.from("TERRA_EGO_UNAUTHORIZED_TAKEOVER")) && asarBytes.includes(Buffer.from("TERRA_EGO_SELECT_FILLINPUT_DENIED")) && asarBytes.includes(Buffer.from("selectOptionByKeyboard")) && asarBytes.includes(Buffer.from("clickPageCommitControl")) && asarBytes.includes(Buffer.from("scrollIntoView")) && asarBytes.includes(Buffer.from("inViewport")), "Packaged app must reject scripted submit, bare Save/Continue clicks, native-alert OK clicks, same-round fill-after-dialog, synthetic DOM events, unauthorized takeOver, and select fillInput; must ship selectOptionByKeyboard and clickPageCommitControl with scroll-into-viewport")
