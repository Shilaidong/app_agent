...
7b. 若保存触发原生 alert（如 “Major is required.”）：结束 ego heredoc → record_blocker（dialogType: alert）→ dismiss_js_alert → 按返回 message 补字段 → 复查 → 重试 Save/Continue。禁止无选择器点击页面「确定」，禁止本回合反复 CDP 关窗。
