...
const MAX_DISMISS_JS_ALERT_ATTEMPTS = 3
const MAX_DISMISSED_JS_ALERTS = 40
...
export const EGO_OBSERVE_PAGE_ACTION_SOURCE = `async function observePageAction(action, { actionTimeoutMs = 8000, settleMs = 2000, pollMs = 100, pageInfoTimeoutMs = 1500 } = {}) {
  let settled
...
if [ "\${1:-}" = "nodejs" ]; then
  ...
  if /usr/bin/grep -Eiq 'reload' "$EGO_NODE_STDIN_COMPACT"; then
    printf '%s\\n' 'TERRA_EGO_UNSAFE_PAGE_RELOAD: 申请页面异常或本页落盘未确认时禁止自动刷新；请结束本回合，下一回合只重新观察或走疑似弹窗通道，仍异常则达限后交接顾问。' >&2
    exit 81
  fi
...
' "$WORKSPACE/03_state/application_progress.json" 2>/dev/null || true)
  ...
  if /usr/bin/grep -Eiq "click[[:space:]]*\\([[:space:]]*['\\\"]确定['\\\"]|click[[:space:]]*\\([[:space:]]*['\\\"]OK['\\\"]" "$EGO_NODE_STDIN_COMPACT"; then
    printf '%s\\n' 'TERRA_EGO_NATIVE_ALERT_CLICK_DENIED: 禁止无选择器地 snapshot click「确定/OK」（浏览器原生 alert 不是页面按钮）。读到 dialog.message 后立刻结束本回合，record_blocker(dialogType:alert) 后 dismiss_js_alert。网页模态框请用带 @selector 的 observePageAction click。' >&2
    exit 81
  fi
  # After an alert message is read in-round, do not keep filling/saving in the same heredoc.
  # Match only dialog.message — never kind:dialog. Canonical observePageAction source contains
  # return { kind: 'dialog', ... } and is pasted into upload/save heredocs by policy.
  # Bare info.dialog presence alone is allowed (normal upload checks "if dialog, stop").
  if /usr/bin/grep -Eiq "dialog\\.message" "$EGO_NODE_STDIN_COMPACT" && /usr/bin/grep -Eiq 'fillInput|uploadFile|begin_save_attempt|record_save_verified' "$EGO_NODE_STDIN_COMPACT"; then
    printf '%s\\n' 'TERRA_EGO_ALERT_MUST_END_ROUND: 本 heredoc 已读取 dialog.message 时不得继续 fill/upload/save。记录文案后立刻结束回合，下一回合再补字段或上传。' >&2
    exit 81
  fi
...
async function writeEgoBrowserSkill(base: string, overrides?: OpenCodeResourceOverrides) {
  ...
  await writeGeneratedFile(
    ...
    [
      ...
      "",
      "## Canonical native select helper",
      "",
      "```js",
      EGO_SELECT_OPTION_BY_KEYBOARD_SOURCE,
      "",
...
4. 完整观察协议见 \`.opencode/skills/ego-browser/TERRA_POLICY.md\`（进入浏览器前必须 read）。拿到新空间的数值 taskSpaceId 后，立即再次调用 prepare_ego_task 保存 ID；后续每个浏览器回合都只使用这个 ID。稳定纯文本区先填完再查；分支点后强制再观察；日期用 fillDatePickerByClicks，两种策略失败即 handoff；编号字段禁猜；截图仅导航/保存/语义不足时使用。
...
const JS_ALERT_AX_JXA = ${JSON.stringify(JS_ALERT_AX_JXA)}

${dismissJsAlertViaAxRuntimeSource()}
...
export const cua = {
  ...
  args: inputArg({
    action: { type: "string", enum: ["prepare_ego_task", "resume_ego", "retire_and_rebind_ego_task", "record_observation", "record_field_verified", "record_select_verified", "record_dynamic_form_verified", "begin_save_attempt", "record_save_verified", "record_blocker", "dismiss_js_alert", "handoff_to_consultant", "complete_ego_task", "record_browser_safety_stop", "resolve_browser_safety_stop", "record_failure", "record_saved", "record_upload", "block_high_risk"], description: "ego-browser coordination action. Browser control itself must be done through the official ego-browser skill, not this tool. dismiss_js_alert uses macOS Accessibility outside the Ego CDP channel." },
    ...
    dialogType: { type: "string", enum: ["alert", "confirm", "prompt", "beforeunload"], description: "For record_blocker: native dialog type from pageInfo().dialog.type. Use alert before dismiss_js_alert so pendingJsAlert is recorded." },
    ...
    interactionMethod: { type: "string", description: "Real interaction chain used for field/select verification, such as fillInput+Tab+readback, cdp-key-events+Tab+readback, click+snapshot+click-option+reobserve, or click+keyboard-option+reobserve." },
  ...
  async execute(args, ctx) {
        ...
        return [
          ...
          ...(selectedFreshTaskSpaceId
            ? [
                ...
                "    // Do not Page.handleJavaScriptDialog here — it wedges the Ego channel while a dialog is open.",
                "    const handoff = dialog.type === 'confirm' || dialog.type === 'prompt' ? await handOffTaskSpace(task.id) : undefined",
                "    const nextRound = dialog.type === 'alert'",
                "      ? 'record_blocker(blockerDisposition: resolved, dialogType: alert, evidence: dialog.message) then dismiss_js_alert; never CDP accept'",
                "      : dialog.type === 'beforeunload'",
                "        ? 'end round; next heredoc pageInfo only to confirm URL unchanged; never CDP accept/cancel'",
                "        : 'handoff to consultant; do not takeOver'",
                "    cliLog(JSON.stringify({ taskSpaceId: task.id, kind: 'dialog', dialog, capturedAlerts: navigation.capturedAlerts, cleanupError: navigation.cleanupError, infoError: navigation.infoError, handoff, nextRound }, null, 2))",
          ...
          ...(selectedFreshTaskSpaceId
            ? [
                "如果输出 kind:alerts，完整弹窗文字已经由直接 Ego CDP 记录并以等价“确定”语义关闭；本回合立即结束，不再 snapshot、填写或导航。调用 record_blocker（blockerDisposition: resolved）保存 alerts 证据（不要 dialogType: alert，无需 dismiss_js_alert），下一独立 heredoc 只复用同一 taskSpaceId 并调用 pageInfo。",
                "如果输出 kind:dialog：本回合立即结束，禁止 Page.handleJavaScriptDialog。type 为 alert：record_blocker（blockerDisposition: resolved，dialogType: alert，evidence 为 dialog.message）后 dismiss_js_alert。type 为 beforeunload：下一独立 heredoc 只 pageInfo，确认 URL 未变。type 为 confirm/prompt：handOffTaskSpace 后 handoff_to_consultant，顾问处理期间不 takeOver。",
    ...
    if (input.action === "record_field_verified" || input.action === "record_select_verified") {
      ...
      const permittedMethod = kind === "select"
        ? interactionMethod === "click+snapshot+click-option+reobserve" || interactionMethod === "click+keyboard-option+reobserve"
      ...
      const auditError =
        ...
        (!permittedMethod ? "REAL_INTERACTION_REQUIRED: fields require fillInput+Tab+readback or per-key CDP+Tab+readback; selects require click+snapshot+click-option+reobserve or click+keyboard-option+reobserve (selectOptionByKeyboard)." : "") ||
        ...
        (kind === "select" && /fillinput/i.test(interactionMethod) ? "DIRECT_PAGE_WRITE_FORBIDDEN: fillInput is not a select verification method; use click+snapshot+click-option+reobserve or click+keyboard-option+reobserve." : "") ||
    ...
    if (input.action === "dismiss_js_alert") {
      ensureCuaProgress(progress)
      const taskSpaceId = String(input.taskSpaceId || progress.egoBrowser?.taskSpaceId || "").trim()
      ...
      if (auditError) {
        appendLimited(progress, "failedActions", { at: new Date().toISOString(), action: auditAction, reason: auditError, page: progress.currentPage || "" })
        await writeJson(join(workspace, "03_state/application_progress.json"), progress)
        await appendLog(workspace, "cua", "拒绝 dismiss_js_alert：" + auditError)
        await appendAudit(workspace, "cua", auditAction, "failed", auditError)
        return auditError
      }
      ...
      if (ax.dismissed) {
        progress.egoBrowser = {
          ...(progress.egoBrowser || {}),
          lastAlertMessage: ax.message || pending.message || "",
    ...
    if (input.action === "record_blocker") {
      ...
      const dialogType = String(input.dialogType || "").trim()
      const pageTitle = String(input.pageTitle || progress.currentPage || "").trim()
      const evidence = String(input.evidence || input.text || "").trim()
    ...
    if (input.action === "handoff_to_consultant") {
      ...
      const pageTitle = String(input.pageTitle || progress.currentPage || "").trim()
      const evidence = String(input.evidence || input.text || "").trim()
      const detailText = String(input.detail || "")
      const evidenceBlob = evidence + " " + detailText
      const pendingAlert = progress.egoBrowser?.pendingJsAlert
      const alertPending = pendingAlert?.type === "alert"
      const controlLossKindProbe = input.controlLossKind === "unexpected_control_loss" || /TERRA_EGO_TASKSPACE_CONTROL_LOST/.test(evidenceBlob)
      ...
      const handoffBlocked =
        alertPending && !alertDismissHandoffAllowed(progress)
          ...
          : timeoutWithoutSuspectedPath || openSaveAttemptBlocked
      const auditError =
        handoffBlocked ||
        browserAuditError(auditAction, { taskSpaceId, currentUrl, pageTitle, evidence }) ||
        requireNumericTaskSpaceId(taskSpaceId) ||
      ...
      const controlLossKind = controlLossKindProbe
