...
describe("application refill state", () => {
  ...
  test("writes a refill-agent config that can only mutate browser execution state through approved tools", async () => {
    ...
    expect(agent.model).toBe("opencode-go/qwen3.7-plus")
    ...
    expect(agent.permission.read["05_screenshots/**"]).toBe("allow")
    ...
    expect(agent.permission.bash["ego-browser nodejs*"]).toBeUndefined()
  ...
  test("allows exact read-only access to a student shared dossier from a nested school workspace", async () => {
    ...
    expect(primary.bash["ego-browser nodejs*"]).toBeUndefined()
    expect(primary.bash["*>*"]).toBeUndefined()
